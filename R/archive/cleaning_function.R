...
end_tonext_duration <- function(df, pointer, unit="mins"){
  difftime(as.POSIXct(ymd_hms(df[["end"]][pointer])), as.POSIXct(ymd_hms(df[["start"]][pointer+1])), units = unit)}
start_tonext_duration <- function(df, pointer, unit="mins"){
  difftime(as.POSIXct(ymd_hms(df[["start"]][pointer+1])), as.POSIXct(ymd_hms(df[["start"]][pointer])), units = unit)}
...
survey_tonext_check <- function(df){
  list_byenum <- df %>% split(.$global_enum_id) %>% map(~.[order(.$start),])
  res<-map(list_byenum, survey_tonext_loop) 
...
other_check<-function(dbs,survey){
  ...
  oth<-which((str_detect(names(dbs),"_autre")|str_detect(names(dbs),"_autre$"))&( sapply(dbs,class)=="factor"|sapply(dbs,class)=="character"))
